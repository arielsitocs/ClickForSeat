...
export default function SeatsResume({ status, setStatus, seats, setSelectedSeats, show }: SeatsResumeTypes) {
    ...
    const handleDeleteSeat = (id: number) => {
        const newSeats = seats.filter((seat: SeatTypes) => seat.id !== id);
    ...
    return (
        status ? (
            <div className="flex items-center justify-center fixed top-0 left-0 right-0 bottom-0 bg-black/50">
                <div className="grid lg:grid-cols-[70%_30%] items-center gap-4 px-6 py-4 bg-darkest-gray rounded-[5px] w-[95%] min-h-[50%] max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="flex items-center flex-col bg-gray p-3 rounded-[5px] h-full w-full lg:col-start-1 lg:row-start-1">
                        <h1 className="text-center bg-darkest-gray p-4 lg:w-[30%] md:w-[60%] w-[80%] rounded-[5px] font-bold">Asientos Seleccionados</h1>
                    ...
                    <div className="flex flex-col items-center bg-gray p-3 rounded-[5px] h-full w-full lg:col-start-2 lg:row-start-1 lg:row-span-2">
                        <h1 className="text-center bg-darkest-gray p-4 rounded-[5px] lg:w-[50%] md:w-[70%] w-[80%] lg:mb-0 mb-6 font-bold">Resumen de Pago</h1>
                        ...
                        <div className="flex flex-col items-center justify-center mt-auto">
                            <hr className="w-[100%] border-2 rounded-[3px] lg:mt-0 mt-6" />
                            <h1 className="mt-6 mb-6 font-bold">Total: ${Math.round(total + total * iva)}CLP</h1>
                            <div className="flex items-center justify-center gap-2">
                                <Button func={() => { }} text="PAGAR" bg_color="bg-electric-blue" text_color="text-white" width="50%" height="40px" font_size="text-sm" />
                                <Button func={() => setStatus(false)} text="CANCELAR" bg_color="bg-darkest-gray" text_color="text-white" width="50%" height="40px" font_size="text-sm" />
                            </div>
                        </div>
                    </div>
